...
func (s *Service) GetScanCountDebug(scanID int) (map[string]interface{}, error) {
	// Count scan results in database
	var count int
	err := s.db.QueryRow("SELECT COUNT(*) FROM scan_results WHERE scan_id = ?", scanID).Scan(&count)
	if err != nil {
		return nil, err
	}

	// Get sample results
	rows, err := s.db.Query("SELECT id, endpoint_type, url, method FROM scan_results WHERE scan_id = ? LIMIT 5", scanID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		sampleResults = append(sampleResults, map[string]interface{}{
			"id":            id,
